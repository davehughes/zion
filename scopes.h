...
struct scope_t : public std::enable_shared_from_this<scope_t> {
	...
	virtual bound_type_t::ref get_bound_type(atom type_name) = 0;
...
struct scope_impl_t : public BASE {
	...
	bound_type_t::ref get_bound_type(atom type_name);
...
struct program_scope_t : public module_scope_impl_t {
	...
	virtual bound_type_t::ref get_bound_type(atom type_name);
	void put_bound_type(status_t &status, bound_type_t::ref type);

	// REVIEW: probably don't need this for LLZ
	void put_bound_type_mapping(status_t &status, atom source, atom dest);
...
private:
	...
	std::map<atom, atom> bound_type_mappings;
	   	...
	   	bound_var_t::ref bound_variable)
{
	...
	auto type_name = bound_variable->get_type_name();
	auto existing_bound_var_iter = resolve_map.find(type_name);
	if (existing_bound_var_iter != resolve_map.end()) {
		...
		user_error(status, bound_variable->get_location(),
			   "failed to bind %s as its name and type_name are already taken",
			   bound_variable->str().c_str());
		...
		resolve_map[type_name] = bound_variable;
...
template <typename T>
bound_type_t::ref scope_impl_t<T>::get_bound_type(atom type_name) {
